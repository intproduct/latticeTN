#!/usr/bin/env python3
"""Stage 7C spinful Hubbard chain benchmark runner.

Benchmarks the three AD mainline solvers on the open-boundary 1D spinful
Hubbard chain on CPU and (opt-in) a V100/TITAN V GPU:
...
GPU rules (Stage 7C, unified selector -- scripts/gpu_selector.py):
...
Conventions: open-boundary spinful Hubbard chain,
    H = -t sum_{i,s} (c^d_{i,s} c_{i+1,s} + h.c.)
        + U sum_i (n_{iup}-1/2)(n_{i,down}-1/2)
        - mu sum_i (n_{i,up}+n_{i,down}-1) - h sum_i (n_{i,up}-n_{i,down}),
d=4, complex128, local basis |0>,|up>,|down>,|up,down>, site-major global
ordering (0_up,0_down,1_up,1_down,...). ED is CPU-only reference baseline.
This is 1D Jordan-Wigner fermions, NOT a full graded fermionic tensor network.
"""
...
def fast_config() -> BenchmarkConfig:
    ...
    return BenchmarkConfig(
        cases=[
            CaseSpec(
                N=4, chi=4, t=1.0, U=4.0, mu=0.0, h=0.0,
            ...
            CaseSpec(
                N=4, chi=8, t=1.0, U=4.0, mu=0.0, h=0.0,
...
def _exact(spec: CaseSpec) -> float:
    H = hubbard_dense(spec.N, t=spec.t, U=spec.U, mu=spec.mu, h=spec.h,
...
def _build_mpo(spec, device):
    return MPO.from_bonds(spec.N, 4, dtype=DTYPE, device=device). \
        generate_hubbard(t=spec.t, U=spec.U, mu=spec.mu, h=spec.h)
...
def run_global_ad(spec, device, seed):
    ...
    return {"solver": SOLVER_GLOBAL_AD, "optimizer": "adam",
            "N": spec.N, "chi": spec.chi, "t": spec.t, "U": spec.U,
            "mu": spec.mu, "h": spec.h, "seed": seed,
...
def run_benchmark(config: BenchmarkConfig) -> dict:
    ...
    report = {
        "convention": (
            "H = -t sum_{i,s}(c^d_{i,s} c_{i+1,s}+h.c.) + U sum_i "
            "(n_{i,up}-1/2)(n_{i,down}-1/2) - mu sum_i (n_{i,up}+n_{i,down}-1)"
            " - h sum_i (n_{i,up}-n_{i,down}); open boundary; d=4; complex128; "
            "local basis |0>,|up>,|down>,|up,down>; site-major global ordering "
            "(0_up,0_down,1_up,1_down,...); 1D Jordan-Wigner fermions (NOT "
            "graded fermionic tensors)."),
        "dtype": str(DTYPE),
        "seed": config.seed,
        "mainline_statement": (
            "This benchmark evaluates the AD mainline solvers (global AD-MPS, "
            "one-site AD local, two-site AD local) on the spinful Hubbard "
            ...
            "only the Hamiltonian/MPO/operator layer is new (Stage 7C)."),
        ...
        "known_limitations": [
            ...
            "network. No TDVP, no finite-temperature, no graded tensors.",
            "Open-boundary spinful Hubbard chain only (d=4); no long-range "
            "hopping / interaction.",
            ...
            "solver optimization.",
            "The dense reference hubbard_dense builds the global operators with "
            "the full 2N-mode Jordan-Wigner parity string; it is a genuine "
            "fermionic Hamiltonian, NOT a spin / hard-core-boson one.",
        ],
    }
    ...
    for spec in config.cases:
        ...
        if gpu_skip_reason is None:
            def _gpu(spec, runner, label):
                ...
                mps_cpu = MPS(spec.N, 4, spec.chi, dtype=DTYPE, device="cpu")
                mps_gpu = MPS(spec.N, 4, spec.chi, dtype=DTYPE,
                              device=gpu_device)
                ...
                row = {"solver": label, "optimizer": r.get("optimizer", "?"),
                       "N": spec.N, "chi": spec.chi, "t": spec.t, "U": spec.U,
                       "mu": spec.mu, "h": spec.h, "seed": config.seed,
                       "final_energy": float(r["final_energy"]),
                       "initial_energy": float(r["initial_energy"]),
...
def render_report_md(report: dict) -> str:
    L = []
    L.append("# Stage 7C Spinful Hubbard Chain Benchmark Report")
    L.append("")
    L.append("Generated by `scripts/run_hubbard_benchmark.py`.")
    L.append("")
    ...
    L.append("This is **1D Jordan-Wigner fermions**, NOT a full graded "
             "fermionic tensor network. The JW parity string (per-site "
             "`P = F_up x F_down = (-1)^{n_up+n_down}` on the left-factor "
             "site of each spin-resolved hop, plus the intra-site `F_up` "
             "inside `cdown`/`cdagdown`) is the key. The AD loss path is "
             "unchanged; Stage 7C adds only the Hamiltonian/MPO/operator "
             "layer. Exact diagonalization is a reference baseline only.")
    L.append("")
    ...
    L.append("")
    L.append("| N | t | U | mu | h | exact E0 | ED runtime_s |")
    L.append("|---:|---:|---:|---:|---:|---:|---:|")
    for c in report["cases"]:
        ex = c["exact"]["final_energy"]
        L.append(f"| {c['N']} | {c['t']} | {c['U']} | {c['mu']} | {c['h']} | "
...
def main() -> int:
    ...
    parser.add_argument("--markdown-output", type=Path,
                        default=ROOT / "docs" / "HUBBARD_REPORT.md")
    ...
    print(f"hubbard benchmark: pass={report['pass']} "
