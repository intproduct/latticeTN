"""Stage 7A: CPU/GPU energy parity + timing for the fermion t-V chain.
...
- records runtime / speedup per the Stage 7A reporting contract (device name,
...
def _build_mpo(N, t, V, mu, device):
    return MPO.from_bonds(N, 2, dtype=DTYPE, device=device).generate_spinless_fermion(
        t=t, V=V, mu=mu)
...
def _run_global_ad(N, chi, t, V, mu, seed, device, steps=80):
    ...
    mpo = _build_mpo(N, t, V, mu, device)
    ...
    return {
        ...
        "N": N, "chi": chi, "t": t, "V": V, "mu": mu, "seed": seed,
...
def test_gpu_selection_respects_v100_titan_filter():
    ...
    sel = select_gpu()
    # When skipped, device is None — correct.
    if sel.skip_reason is not None:
        assert sel.device is None
        return
    # When a device is selected, its name must contain an allowed filter.
...
def test_cpu_global_ad_lowers_energy_and_not_below_ground():
    ...
    for N, chi in [(4, 4), (6, 8)]:
        t, V, mu = 1.0, 0.5, 0.0
        e0 = _exact_e0(N, t, V, mu)
        res = _run_global_ad(N, chi, t, V, mu, seed=1, device="cpu")
...
def test_cpu_gpu_energy_parity_and_timing():
    ...
    for N, chi in [(4, 4), (6, 8)]:
        t, V, mu = 1.0, 0.5, 0.0
        e0 = _exact_e0(N, t, V, mu)
        cpu = _run_global_ad(N, chi, t, V, mu, seed=1, device="cpu")
        gpu = _run_global_ad(N, chi, t, V, mu, seed=1, device=device)
        # parity
