...
def install_safe_lanczos_cpu_eigh() -> None:
    """Patch latticeTN Lanczos with a robust SciPy tridiagonal eigensolver.

    The DMRG tensors and Heff apply still run on the requested device, e.g. CUDA.
    Only the tiny m x m Lanczos tridiagonal matrix, with m <= --max-iter, is
    represented on CPU as two float64 arrays and solved by scipy.linalg.eigh_tridiagonal.
    This avoids both CUDA and Torch CPU ``torch.linalg.eigh`` failures on nearly
    degenerate/ill-conditioned Ritz matrices.
    """
    import numpy as np
    import torch as tc
    ...
    try:
        ...
        raise RuntimeError(
            "This robust Lanczos patch needs scipy. Install it with: pip install scipy"
        ) from exc

    def _lowest_tridiag_eigenpair(alphas, betas):
        """Return lowest eigenvalue/eigenvector of real symmetric tridiagonal T.

        alphas: length m diagonal.
        betas:  length >= m-1 off-diagonal entries; extra final beta is ignored.
        """
        diag = np.asarray(alphas, dtype=np.float64)
        m = int(diag.size)
        if m == 0:
            raise RuntimeError("Lanczos produced an empty Krylov basis")
        if m == 1:
            return float(diag[0]), np.ones(1, dtype=np.float64)

        off = np.asarray(betas[: m - 1], dtype=np.float64)

        # Clean tiny/invalid numerical garbage rather than feeding LAPACK NaNs.
        if not np.all(np.isfinite(diag)) or not np.all(np.isfinite(off)):
            raise FloatingPointError(
                "Lanczos tridiagonal contains NaN/Inf; try dtype=complex128, "
                "smaller --max-iter, or smaller --tol."
            )
        tiny = 100.0 * np.finfo(np.float64).eps * max(1.0, float(np.max(np.abs(diag))))
        off[np.abs(off) < tiny] = 0.0

        # Best path: specialized symmetric-tridiagonal solver, selected lowest pair.
        # stebz is usually more forgiving for clustered eigenvalues than the default.
        try:
            w, v = eigh_tridiagonal(
                diag,
                off,
                eigvals_only=False,
                select="i",
            ...
            return float(w[0]), np.asarray(v[:, 0], dtype=np.float64)
        except Exception:
            pass

        # Fallback 1: compute the full tridiagonal spectrum with a different driver.
        try:
            w, v = eigh_tridiagonal(
                diag,
                off,
        ...
        # Fallback 2: dense NumPy on the tiny T, with escalating diagonal jitter.
        T = np.diag(diag)
        T += np.diag(off, k=1) + np.diag(off, k=-1)
        scale = max(1.0, float(np.max(np.abs(T))))
        last_exc = None
        for eps_factor in (0.0, 1e-14, 1e-12, 1e-10):
            try:
                T2 = T if eps_factor == 0.0 else T + (eps_factor * scale) * np.eye(m)
                w, v = np.linalg.eigh(T2)
                return float(w[0]), np.asarray(v[:, 0], dtype=np.float64)
            except Exception as exc:
                last_exc = exc
        raise RuntimeError("All tridiagonal eigensolver fallbacks failed") from last_exc
    ...
    def safe_lanczos_lowest_eigenpair(
        ...
        def _run_once(v0: tc.Tensor):
            ...
            for j in range(max_iter_eff):
                ...
                # Ritz convergence check, solved by SciPy tridiagonal routine.
                m = len(alphas)
                if m >= 2:
            ...
            E0_float, coeff_np = _lowest_tridiag_eigenpair(alphas, betas)
            E0 = tc.tensor(E0_float, dtype=tc.float64, device=device)
            coeffs = tc.as_tensor(coeff_np, dtype=tc.float64, device=device)
            vec = sum(coeffs[k] * Q[k] for k in range(len(coeff_np)))
...
def main() -> int:
    ...
    p.add_argument("--safe-lanczos", action="store_true", default=True, help="diagonalize the tiny Lanczos tridiagonal T matrix with SciPy on CPU to avoid Torch/CUDA eigh failures")
    ...
    if args.safe_lanczos:
        ...
        print("Safe Lanczos patch: ON, Krylov T matrix uses SciPy eigh_tridiagonal on CPU; DMRG tensors stay on the selected device.")
